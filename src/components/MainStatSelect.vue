<template>
  ...
  ></ul>
  <ul
    :class="style(getValue(selected_stat))"
    class="art-stats__stats-list"
  >
    <!-- Выбранная характеристика -->
    <li
      class="art-stats__selected-stat"
      @click="hide"
    >
      <p class="image_align">
...
function setSelect(stat: Stat) {
  ...
  // @ts-ignore
  selected_stat.value[stat.name] = stat.value;
}
